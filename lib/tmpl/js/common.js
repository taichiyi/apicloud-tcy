...
define(['../js/ezapp'], function() {
    ...
    */
    ...
    F._open_nav = function() {
        function appInstalled(callback) {
            var appBundle;
            if (Q.isIOS()) {
                appBundle = 'iosamap://';
                // appBundle = 'weixin://';
                // appBundle = 'sinaweibo://';
                // appBundle = 'iosamap://navi';
                // appBundle = 'mqq://';
            } else {
        ...
        function open() {
            ...
            // uri += '&poiname=';
            // uri += '&lat=' + destinationLat + '&lon=' + destinationLon;
            // uri += '&dev=0';
            // uri += '&style=2';
    ...
    F._openWin = function(elem) {
        var i;
        var self = $(elem);
        var name = self.data('open-name');
        var cache = self.data('open-params');
        var access = self.data('open-access'); // 权限
        var check_login = self.data('open-check-login') + ''; // 检查是否登录
        var options = self.data('open-options'); // api.openWin其他选项
        var default_json;
        ...
        if (access === '' || access === undefined) {
            ...
            if (check_login === 'false' || check_login === 'undefined') {
                // 如果没有设置是否权限验证, 则直接通过权限验证
                check_login = false;
            } else {
                check_login = true;
            }
        } else {
            // 如果设置了权限等级, 则不能直接通过权限验证
            check_login = false;
        }
        console.log(check_login);

        if (access === '' || access === undefined) { //判断变量是否添加        
            access = '0';
        } else {
            access += '';
        }
            ...
            switch (access) {
                ...
                case '0':
                    ...
                    if (Q.isIOS()) {
                        ...
                        default_json.delay = 240;
                    }
                    default_json.name = 'login-reg';
                    default_json.bgColor = 'widget://img/d89184.png';
    ...
    Q.ready(function() {
        // 点击打开windows
        $('.gazer-role-open').live(F._tap, function() {
            var i;
            var that = $(this);
            var name = that.data('open-name');
            var cache = that.data('open-params');
            var options = that.data('open-options');
            var default_json;
        ...
        // 点击运行函数
        $('.js_func').live(F._tap, function() {
            var that = $(this);
            var name = that.data('func-name');
